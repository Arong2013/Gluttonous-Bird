...
public class RecoveryItem: CountableItem, IUseableItem
{
    ...
    {
        foreach (var recoveryStat in recoveryStats)
        {
            var statName = recoveryStat.Key;
            var recoveryAmount = recoveryStat.Value;
            playerMarcine.characterData.UpdateStat(statName, this, recoveryAmount);
        }
    }
}